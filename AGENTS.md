...
- **Game code is one `UIAction`** — `demo.setLabel`, registered in `runtime/setup.ts`
  (~35 lines). The showcase timeline's signal track calls it once per station: it sets the
  ...
  layout.
- **Scene** (`runtime/assets/scenes/main.json`, 27 entities) — one scene. A `Main Camera`
  ...
  driving `showcase.timeline.json`, 14 `ParticleEmitter` entities (mostly parented under
...
- **Particle assets** (`runtime/assets/particles/*.particle.json`, 18 files — 14 wired +
...
- **2D gallery** (`runtime/assets/scenes/2d-gallery.json`) — a separate scene showing the
...
- **Engine changes made for this demo** (not demo-local — they ship with the engine):
  `BloomPostFX` trait + `BloomPostProcess` (`runtime/rendering/bloom/`, WebGPU/TSL
  whole-scene bloom on the plain forward-render path, wired into `Scene3D`'s render
  branch), and a `meshParticles.ts` primitive-tessellation bump (torus 8×16 → 16×80 +
  thinner tube; sphere/cone up) so mesh-mode particles (Debris) aren't visibly faceted.
  Both are documented in `docs/rendering.md` / the engine's trait docs, not here.
...
- Build/run: open in the Modoki Editor (**File → Open Project**), then **Build → Web**, or
  `MODOKI_PROJECT=demos/particle-demo npm run build` from the repo root.
- **Bloom is WebGPU-only.** On a WebGL2 fallback the plain render path is used and the
  scene loses its glow — still renders correctly, just flatter.
